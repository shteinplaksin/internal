# AssaultCube OpenGL Overlay DLL - Техническая документация
...
Данная DLL представляет собой внутриигровой оверлей для игры AssaultCube (x86/32-bit), использующей OpenGL. Проект реализует внедрение пользовательского графического интерфейса через перехват OpenGL API с применением техник стелс-инжекта и обфускации.
...
**Когда удаляем из PEB?**
```cpp
if (IsAssaultCubeProcess())
    RemoveFromPEB(hModule);
```

Процесс AssaultCube не выполняет дополнительных проверок целостности списков модулей, поэтому безопасно удалять DLL из PEB для повышения стелса. Для сторонних приложений (если DLL была загружена вручную для отладки) этот вызов пропускается.
...
- Это особенно полезно для AssaultCube, которая инициализирует OpenGL при первом создании окна
...
bool __stdcall wglSwapBuffers(HDC hDc)
{
    ...
    if (!wnd_handle && current_wnd && IsWindow(current_wnd))
    {
        ...
        GetWindowTextA(current_wnd, window_title, sizeof(window_title));

        if (strcmp(current_class, "SDL_app") == 0 || strstr(window_title, "AssaultCube") != nullptr)
        {
...
**Почему проверяем класс и заголовок окна?**
- AssaultCube использует SDL, и класс окна по умолчанию `"SDL_app"`
- На некоторых сборках заголовок окна содержит строку `"AssaultCube"`
- Проверка обоих признаков гарантирует, что мы привяжемся к игровому окну, даже если класс или заголовок отличаются от стандартного значения
...
- У нас нет контроля над оконной процедурой (WndProc) целевого процесса
...
#### xorstr
```cpp
const char* target = xorstr_("SDL_app");
```
...
5. **Stealth::InitStealth**:
   ...
   - Удаление из PEB (только для AssaultCube)
...
9. При каждом SwapBuffers:
   - Поиск главного окна AssaultCube (один раз)
...
1. **100ms до stealth**:
   ...
   - Критично для стабильности при инжекте в игры

2. **200ms до hook**:
   - После манипуляций с памятью (стирание PE, PEB) системе нужно время
   - OpenGL может еще не быть полностью инициализирован
   - AssaultCube может выполнять отложенную инициализацию графического движка
...
1. **Конкурентный доступ**: Если процесс перечисляет модули во время удаления из PEB → крэш
2. **Integrity checks**: Некоторые приложения могут проверять целостность PEB
3. **Timing**: Слишком ранние манипуляции могут поймать процесс в нестабильном состоянии

### Решения для стабильности

1. **Селективное включение RemoveFromPEB** только для AssaultCube
...
AssaultCube использует OpenGL для рендеринга (движок основан на Cube Engine). Direct3D хуки не сработали бы, так как:
- AssaultCube полностью основана на OpenGL
- Игра не использует Direct3D в своём рендер-пайплайне
- OpenGL-хук является наиболее надёжным способом
...
- AssaultCube 1.x (32-битная версия)
- x86 архитектура
